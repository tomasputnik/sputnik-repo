...
sensor_msgs::PointCloud2 receive(){
	...
	for(i= 256, j=0, k=0; i < Ret; i=i+2, j++, k++){ 
		...
		//y=corrected_range*angulo_servo; tese
		y=corrected_range*sin((pi/180.0)*(starting_angle + j*angle_increment));  //trsa -trab sonar
		z=corrected_range*sin((pi/180.0)*(starting_angle + j*angle_increment));
		...
		pontosXX[k]=x; pontosYY[k]=y; pontosZZ[k]=z;  //guarda todos os pontos para depois fazer a imagem 3D
...
int main(int argc, char **argv)
{
  ...
  sensor_msgs::PointCloud2 msg;
        ...
        sizeof(struct sockaddr)) == -1)
   {
	...
	while(!kbhit()){
		printf(" \n\n\n");
		printf(" Press any key to start 3D scanning...\n");
	}

	printf("Press any key to shutdown sonar\n");
	//UdpOut();
	while(!kbhit())
	{
